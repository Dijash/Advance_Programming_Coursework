...
<%-- My Favorites page --%>
<c:set var="activePage" value="myFavorites" scope="request" />
      ...
      <section class="content-box">
        ...
        <c:choose>
          <c:when test="${not empty favoriteVehicles}">
            <%-- Grid of favorite vehicle cards --%>
            <div class="favorites-grid">
              <c:forEach var="v" items="${favoriteVehicles}">
                <%-- Individual favorite vehicle card --%>
                <div class="fav-card">
                  <img src="${pageContext.request.contextPath}/Assets/${v.vehicle_image}"
                    alt="${v.vehicle_brand}" class="fav-img"
                    onerror="this.src='https://placehold.co/600x400/f1f5f9/94a3b8?text=Vehicle'" />

                  <%-- Vehicle info section --%>
                  <div class="fav-info">
                    <h4>
                      <c:out value="${v.vehicle_brand}" />
                      <c:out value="${v.vehicle_type}" />
                    </h4>
                    <p>
                      <c:out value="${v.vehicle_color}" /> |
                      <c:out value="${v.vehicle_numberPlate}" />
                    </p>
                    ...
                    <div class="fav-actions">
                      <%-- Book now button (disabled if vehicle unavailable) --%>
                      <a href="${pageContext.request.contextPath}/viewVehicleDetails?id=${v.vehicle_id}"
                        class="btn-book" ${v.vehicle_status !='Available'
                        ? 'style="background:#94a3b8; pointer-events:none;"' : '' }>
          ...
          <c:otherwise>
            <div class="empty-state">
              <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="1.5" style="color: #cbd5e1; margin-bottom: 1rem;">
                <path
              ...
              <a href="${pageContext.request.contextPath}/rentalDeals" class="btn-home"
                style="display: inline-block; margin-top: 1rem; color: var(--accent); border-color: var(--accent);">Browse
                Fleet</a>
            </div>
          </c:otherwise>
        </c:choose>
