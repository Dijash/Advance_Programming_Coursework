  ...
  <section>
    ...
    <section class="deals" id="deals">
      <div class="section_container deals_container">
        ...
        <p class="section_description">Explore our top car rental deals, handpicked to give you the best value.
        </p>
        <%-- Featured deal panel --%>
          ...
          <div class="subscribe_content">
            ...
            <c:choose>
              ...
              <c:when test="${param.subscribeStatus == 'duplicate'}">
                <p class="subscribe_flash subscribe_flash--warning">
                  That email is already subscribed. Want to unsubscribe? Enter your email and click
            ...
            <%-- Subscribe or unsubscribe form --%>
            <c:choose>
              <c:when test="${param.subscribeStatus == 'duplicate'}">
                <%-- Show both subscribe and unsubscribe buttons for duplicate emails --%>
    ...
    <section class="section_container client_container" id="client">
      ...
      <div class="client_grid">
        <c:choose>
                  ...
                  <c:choose>
                    <c:when test="${not empty review.customerImage}">
                      <img src="${pageContext.request.contextPath}/Assets/Profiles/${review.customerImage}"
                        alt="${review.customerName}" class="client_avatar" />
                    </c:when>
                    <c:otherwise>
                      <img src="${pageContext.request.contextPath}/Assets/images.jpeg"
                        alt="${review.customerName}" class="client_avatar" />
                    </c:otherwise>
                  </c:choose>
                  <%-- Reviewer name --%>
          ...
          <c:otherwise>
            <div
              style="grid-column: 1 / -1; text-align: center; padding: 40px; background: #f8fafc; border-radius: 12px;">
              <p style="color: #64748b; font-size: 16px;">No reviews yet. Be the first to rent and leave a
                review!</p>
            </div>
          </c:otherwise>
        </c:choose>
