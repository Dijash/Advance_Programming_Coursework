    ...
    <section>
      ...
      <section class="deals" id="deals">
        <div class="section_container deals_container">
          ...
          <p class="section_description">Explore our top car rental deals, handpicked to give you the best value.</p>
          <div class="deal_panel">
            <div class="deal_image">
              <img src="${pageContext.request.contextPath}/Assets/audi 6.jpg" alt="Audi A6 Sedan" />
            </div>
            <div class="deal_info">
              <h3>Audi A6 Sedan</h3>
              ...
              <div class="deal_price_row">
                ...
                <a href="${pageContext.request.contextPath}/viewVehicleDetails?id=${v.vehicle_id}" class="button">Book Now</a>
              </div>
            </div>
          </div>
            ...
            <div class="subscribe_content">
              ...
              <c:choose>
                ...
                <c:when test="${param.subscribeStatus == 'duplicate'}">
                  <p class="subscribe_flash subscribe_flash--warning">
                    That email is already subscribed. Want to unsubscribe? Enter your email and click &ldquo;Inactive&rdquo;.
                  </p>
                </c:when>
              ...
              <c:choose>
                <c:when test="${param.subscribeStatus == 'duplicate'}">
                  <div class="subscribe_form" style="gap:0.5rem; flex-wrap:wrap;">
                    <form action="${pageContext.request.contextPath}/subscribe" method="post" style="display:contents;">
                      <input type="hidden" name="action" value="subscribe" />
                      <input type="email" name="email" value="${param.email}" placeholder="Enter your email address" required />
                      <button type="submit" class="button">Subscribe</button>
                    </form>
                    <form action="${pageContext.request.contextPath}/subscribe" method="post" style="display:contents;">
                      <input type="hidden" name="action" value="unsubscribe" />
                      <input type="hidden" name="email" value="${param.email}" />
                      <button type="submit" class="button" style="background:#fee2e2; color:#dc2626; border:1px solid #fca5a5;">
                ...
                <c:otherwise>
                  <form action="${pageContext.request.contextPath}/subscribe" method="post" class="subscribe_form">
                    <input type="hidden" name="action" value="subscribe" />
          ...
          <c:choose>
            <c:when test="${not empty recentReviews}">
              <c:forEach var="review" items="${recentReviews}">
                <div class="client_card">
                  <div class="client_details">
                    <c:choose>
                      <c:when test="${not empty review.customerImage}">
                        <img src="${pageContext.request.contextPath}/Assets/Profiles/${review.customerImage}" alt="${review.customerName}" class="client_avatar" />
                      </c:when>
                      <c:otherwise>
                        <img src="${pageContext.request.contextPath}/Assets/images.jpeg" alt="${review.customerName}" class="client_avatar" />
                      </c:otherwise>
                    </c:choose>
                    <div>
                  ...
                  <small style="color: #64748b; display: block; margin-top: 10px; font-size: 12px;">
                    Posted on: <fmt:formatDate value="${review.reviewDate}" pattern="MMM dd, yyyy" />
                  </small>
                </div>
              </c:forEach>
            </c:when>
            <c:otherwise>
              <div style="grid-column: 1 / -1; text-align: center; padding: 40px; background: #f8fafc; border-radius: 12px;">
                <p style="color: #64748b; font-size: 16px;">No reviews yet. Be the first to rent and leave a review!</p>
              </div>
            </c:otherwise>
          </c:choose>
        </div>
